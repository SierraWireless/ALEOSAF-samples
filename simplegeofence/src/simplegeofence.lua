...
local control = {
	started        = true;  -- Proceed with fence checking and tracking? Yes
...
function M.check_for_fence_breaches (gps_data)
	...
	if not control.started then
		state.last_reported_position = nil
		return 'ok'
	end
...
function M.handle_command_setfence (fence_string)
	...
	log('GEOFENCE','INFO', 'Convert the fence string to as area: %s',fence_string)
