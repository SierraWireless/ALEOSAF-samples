...
-- @return distance between p1 and p2, in km/h, or nil+error_message.
function M.speed(p1, p2)
    local t1, t2 = p1.timestamp, p2.timestamp
    if not (t1 and t2) then return nil, 'no timestamps' end
...
-- @param #point
-- @return #number
-- @usage geoloc.distance(apoint, anotherpoint) == apoint:distance(anotherpoint)
...
-- @return #number
-- @usage geoloc.speed(apoint, anotherpoint) == apoint:speed(anotherpoint)
point.speed = M.speed
...
function inside.union(p)
    for _, area in pairs(self.operands) do
        if area :contains(p) then return true end
    end
    return false
end
...
function build.circle(self, points)
    ...
    if #p ~= 1 then error "Invalid circle center" end
    if not r   then error "Invalid circle radius" end
    self.center = p[1]
...
function M.newarea(spec)
    ...
    if not kind then return nil, "Invalid shape" end
    local buildk = build[kind]
    if not buildk then return nil, "Invalid shape kind" end
